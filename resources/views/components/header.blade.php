...
<nav class="bg-white dark:bg-gray-900 fixed w-full z-20 top-0 start-0 border-b border-gray-200 dark:border-gray-600">
  ...
  <a href="{{ route('post.index') }}" class="flex items-center space-x-3 rtl:space-x-reverse">
      <img src="{{ asset('img/rachel-icon.png') }}" class="h-8" alt="Rachel icon">
      <span class="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">Rachel Scan</span>
  </a>
  <div class="flex md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
      @auth
        @include('components.profile-drop')   
      @endauth
      @guest
